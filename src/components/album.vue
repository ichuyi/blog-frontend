<template>
    <el-card id="album">
        ...
        <ImageCard v-for="(image,index) in images" :image="image" :key="index" class="image-card" @deleteOne="deleteOne(index,image)" @preview="preview(image.id)"/>
        <div class="preview" v-show="show" :class="{show:show,hidden:!show}">
            ...
            <a :href="`/api/file/get?id=${previewId}`">
                <el-image :src="`/api/file/get?id=${previewId}`" fit="none" class="image"/>
            </a>
        </div>
    </el-card>
</template>
    ...
    export default {
        ...
        methods:{
            ...
            deleteOne(index,image){
                let _self=this;
                http.fetchGet("/file/delete",{id:image.id}).then(function (res) {
                    if(res.data.code!==0) {
